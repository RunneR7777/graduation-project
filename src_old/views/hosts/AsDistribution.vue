<template>
  <div class="as-distribution-container">
    <v-container fluid>
      <v-card>
        <v-card-title>
          自治系统分布
        ...
        >
          <template v-slot:item.name="{ item }">
            <a :href="`https://bgp.he.net/AS${item.asNumber}`" target="_blank" class="text-decoration-none">
              {{ item.name }}
...
export default {
  ...
  data() {
    return {
      ...
      itemsPerPage: 10,
      headers: [
        { text: 'AS编号', value: 'asNumber', align: 'start' },
  ...
  methods: {
    async fetchAsData() {
      this.loading = true;
      try {
        const response = await hostsApi.getAsDistribution();
        
        console.log('AS分布API响应:', response);
        
        if (response.data && response.data.status && response.data.status.code === 200) {
          this.asData = response.data.data.items.map(item => ({
            asNumber: item.asNumber,
            hosts: item.hosts,
            name: item.name || `AS${item.asNumber}`,
            seenSince: item.seenSince,
            sentPercentage: item.sentPercentage,
          ...
          console.error('获取AS分布数据失败:', response);
          this.$message.error('获取AS分布数据失败: ' + (response.data?.status?.message || '未知错误'));
        }
      } catch(error) {
        console.error('获取AS分布数据出错:', error);
